...
``` output
用法: freqtrade install-ui [-h] [-v] [--no-color] [--logfile FILE] [-V]
                           [-c PATH] [-d PATH] [--userdir PATH] [--unstable]
                           [--base-url BASE_URL]

选项:
  -h, --help            显示此帮助消息并退出
  --unstable            安装 FreqUI 的不稳定/包含最新功能的“develop”版本。
  --base-url BASE_URL   FreqUI 的基础 URL。通常不应更改。
