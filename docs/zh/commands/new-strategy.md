...
``` output
用法: freqtrade new-strategy [-h] [-v] [--no-color] [--logfile FILE] [-V]
                             [-c PATH] [-d PATH] [--userdir PATH] [-s NAME]
                             [--strategy-path PATH] [--template {minimal,full}]

选项:
  -h, --help            显示此帮助消息并退出
  -s, --strategy NAME   指定要创建的新策略类名。
  --strategy-path PATH  指定输出策略文件的目录路径（默认：`user_data/strategies`）。
  --template {minimal,full}
                        指定要用于新策略的模板（默认：`minimal`）。
