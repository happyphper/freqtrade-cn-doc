...
选项:
  ...
  --margin-mode {isolated,cross}
                        指定杠杆模式（默认：`isolated`）。
  --base BASE           根据基础货币（Base currency）进行过滤。正则可用（例如：`BTC,*`）。
  --quote QUOTE         根据计价货币（Quote currency）进行过滤。正则可用（例如：`USDT,BTC`）。
  --print-json          打印 JSON 格式的结果。
  --columns COLUMNS [COLUMNS ...]
                        指定要在输出表中显示的列。
                        可用列有：'symbol', 'base', 'quote', 'settle', 'type', 
                        'active', 'contract_size', 'leverage'。
